...
data class CookKitchenUiState(
    val isLoading: Boolean = false,
    val orders: List<Order> = emptyList(),
    val error: String? = null
)

class CookKitchenViewModel(application: Application) : AndroidViewModel(application) {

    private val context = application.applicationContext
    private val orderRepository = OrderRepository(context)
    private val authRepository = AuthRepository(context)

    private val _uiState = MutableStateFlow(CookKitchenUiState())
...
//        }
        ...
        SocketClient.onOrderItemAdded { json ->
            ...
            viewModelScope.launch {
                val current = _uiState.value.orders.toMutableList()
                val index = current.indexOfFirst { it.id == order.id }
                if (index != -1) current[index] = order
                _uiState.value = _uiState.value.copy(orders = current)
            }
        }
    }
